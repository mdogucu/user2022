...
- Cost of books and supplies for a student at a four-year university in 2021 was estimated to be $1240 per year ([Hanson, 2021](https://educationdata.org/average-cost-of-college-textbooks)) in the United States. 
...
```{r echo=FALSE, fig.alt="A timeline titled brief history of knitr. That points out that the initial commit was in October 2011, first download on CRAN was in October 2012, alt-text feature was requested in July 2020, feature was made available in January 2021", fig.align='center'}
...
event <- c("initial commit", "first download on CRAN",
           "alt-text feature requested",
           "feature available")

xend <- lubridate::as_date(c("2010-10-01", "2022-12-31"))
...
Between first CRAN release and alt-text feature request, there were `r prettyNum(sum(logs$count), big.mark = ",", scientific = FALSE)` CRAN downloads of knitr.
...
![:col_header 
  <span style="font-size:50px; color:whitesmoke;">Curriculum Does Not Teach Accessibility </span>,
  <span style="font-size:50px; color:whitesmoke;">Developers Create Inaccessible Tools</span>,
  <span style="font-size:50px; color:whitesmoke;">Data Scientists Create Inacessible Products</span>
              ]
